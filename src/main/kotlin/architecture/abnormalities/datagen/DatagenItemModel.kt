...
class DatagenItemModel(
	 ...
	 */
	fun createModelFile(
		item: Item, prefix: String, textures: Map<Float, String>, parent: ModelFile?,
	 ...
	 * @param predicates 谓词资源位置数组，用于确定何时使用哪个模型变体
	 */
	fun createModelFile(item: Item, prefix: String, textures: Map<Float, String>, vararg predicates: ResourceLocation) {
	...
	private fun getItemResourceLocation(item: Item, name: String): ResourceLocation {
		return BuiltInRegistries.ITEM.getKey(item).withSuffix("_$name")
	}
	...
	fun basicItem(item: Item, name: String): ItemModelBuilder {
		return basicItem(BuiltInRegistries.ITEM.getKey(item), name)
	}
	...
	fun customModelFile(name: String): ModelFile {
		return ModelFile.UncheckedModelFile(Abnormalities.modRl(name))
	}
}
